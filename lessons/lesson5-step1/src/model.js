...
export class Model {
  ...
  getTerminalPoint(terminal) {
    if (terminal.port) return this.getPortPoint(terminal.cell, terminal.port)
    if (terminal.cell) {
  ...
  getEdgeAt(x, y, tolerance = 8) {
    return [...this.edges.values()].reverse().find((edge) => {
      ...
      return source && target && distanceToSegment(x, y, source, target) <= tolerance
    }) || null
  }
  ...
  getNearestPort(x, y, radius = 12, exclude = null) {
    ...
    for (const node of this.nodes.values()) {
      for (const port of node.ports) {
        if (exclude && exclude.nodeId === node.id && exclude.portId === port.id) continue
  ...
  removeNode(id) {
    const node = this.getNode(id)
    if (!node) return null
    for (const edge of this.getConnectedEdges(id)) this.removeEdge(edge.id)
