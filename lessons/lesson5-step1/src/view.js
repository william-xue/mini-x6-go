...
export class SvgView {
  ...
  selectNode(id) {
    if (this.selectedNodeId) this.elements.get(this.selectedNodeId)?.children[0]?.setAttribute('stroke-width', 1)
    this.selectedNodeId = id
    if (id) this.elements.get(id).children[0].setAttribute('stroke-width', 4)
  }

  selectEdge(id) {
    if (this.selectedEdgeId) {
      const oldLine = this.elements.get(this.selectedEdgeId)
      oldLine?.setAttribute('stroke', '#3f78ff')
      oldLine?.setAttribute('stroke-width', 2)
    }
    this.selectedEdgeId = id
    if (id) {
      const line = this.elements.get(id)
      line.setAttribute('stroke', '#f07a28')
