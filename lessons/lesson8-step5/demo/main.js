...
import { Assembly } from '../../lesson8-step4/src/index.js'
...
function renderLibrary() {
  ...
  for (const definition of library.list()) {
    ...
    button.textContent = `${definition.name}（${definition.pins.length} 引脚 · ${definition.size.width.toFixed(0)}×${definition.size.height.toFixed(0)}）`
    button.addEventListener('click', () => {
      const instance = spawn(definition)
      status.textContent = `实例 ${instance.width.toFixed(0)}×${instance.height.toFixed(0)}`
    })
...
document.querySelector('#save').addEventListener('click', () => {
  ...
  renderLibrary()
  status.textContent = `已保存：${definition.size.width.toFixed(0)}×${definition.size.height.toFixed(0)}（已裁切到外形）`
})
